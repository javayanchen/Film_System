...
<script>
  ...
  async function loadData() {
    for (let i = 1; i < 4; i++) {
      try {
        let str = "";
        const data = await $.getJSON("/filmServlet", {tag: "dramaShow", typeId: 4, typeIdId: i}, function (data) {
          const dramaType = i === 1 ? "古装" : (i === 2 ? "动画" : "现代都市");
          str = '<div class="recommended"><div class="recommended-grids"><div class="recommended-info"><div class="heading"><h3>'+dramaType+'</h3>'
                  +'</div><div class="clearfix"></div></div>';
          for (let j = 0; j < data.length; j++) {
            str += '<div class="col-md-2 resent-grid recommended-grid show-video-grid"><div class="resent-grid-img recommended-grid-img">'
                    +'<a href="single.jsp"  onclick="addViews('+ data[j].filmId +')"><img src="'+ data[j].filmImage +'" alt=""/></a><div class="time small-time show-time"><p>'+data[j].filmTime+'</p>'
                    +'</div><div class="clck show-clock"><span class="glyphicon glyphicon-time" aria-hidden="true"></span></div></div>'
                    +'<div class="resent-grid-info recommended-grid-info"><h5><a href="single.jsp" class="title"  onclick="addViews('+ data[j].filmId +')">'+ data[j].filmIntro +'</a></h5>'
                    +'<p class="author"><a href="#" class="author">'+ data[j].filmAuthor +'</a></p><p class="views">'+ data[j].filmViews+1 +'views</p>'
                    +'</div></div>';
          }
