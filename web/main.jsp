<%@ page import="com.film.pojo.User" %><%--
...
<%--                    </script>--%>
                ...
                <div class="signin">
                    <a href="userServlet?tag=logout" class="play-icon popup-with-zoom-anim">退出</a>
<%--                    <script>--%>
<%--                        function logOut() {--%>
<%--                            // 清除用户名--%>
...
<script>
    ...
    async function loadData() {
        for (let i = 4; i < 7; i++) {
            try {
                let str = "";
                const data = await $.getJSON("/filmServlet", {tag: "show", typeId: i}, function (data) {
                    const filmType = i === 4 ? "电视剧" : (i === 5 ? "电影" : "体育");
                    str = "<div class='recommended'><div class='recommended-grids'><div class='recommended-info'><h3>" + filmType + "</h3></div>";
                    for (let j = 0; j < data.length; j++) {
                        str += "<div class='col-md-3 resent-grid recommended-grid'><div class='resent-grid-img recommended-grid-img'><a href='single.jsp' onclick='addViews("+ data[j].filmId +")'>"
                            + "<img src='"+ data[j].filmImage +"' alt=''/></a><div class='time small-time'><p>" + data[j].filmTime + "</p></div>"
                            + "<div class='clck small-clck'>"
                        str += '<span class="glyphicon glyphicon-time" aria-hidden="true"></span></div></div>'
                            + '<div class="resent-grid-info recommended-grid-info video-info-grid"><h5><a href="single.jsp" class="title" onclick="addViews('+ data[j].filmId +')">' + data[j].filmIntro
                            + '</a></h5><ul><li><p class="author author-info"><a href="#" class="author">' + data[j].filmAuthor + '</a></p>' +
